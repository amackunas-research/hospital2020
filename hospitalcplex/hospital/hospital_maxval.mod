...
int p_j[j in 1..n] = ...; // time of the operation
...
subject to {
   forall(k in 1..t, i in M_t[k])
     sum(j in N_it[i][k]) p_j[j] * x_jit[j][i][k] <= D_t[k] + z_it[i][k];
...
int L_it[i in 1..m][k in 1..t] = sum(j in 1..n) x_jit[j][i][k] * p_j[j];
int idl = sum(i in 1..m, k in 1..t) maxl(0, D_t[k] - L_it[i][k]);
...
execute {
  ...
  f.writeln("V_1[" + situation + "][" + instance + "] = " + V_1 + ";");
  f.writeln("time[" + situation + "][" + instance + "] = " + time + ";");
  f.writeln("idl[" + situation + "][" + instance + "] = " + idl + ";");
  f.writeln("");
